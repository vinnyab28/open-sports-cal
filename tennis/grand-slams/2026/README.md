# Tennis Grand Slams 2026

The four Grand Slam tournaments are the most prestigious events in tennis, each spanning approximately two weeks.

Grand Slam events are stored as **all-day multi-day events** covering the full tournament window.

---

## Download / Subscribe

### All 4 Grand Slams

| | |
|--|--|
| **Raw URL** | `https://raw.githubusercontent.com/vinnyab28/open-sports-cal/main/tennis/grand-slams/2026/all-grand-slams.ics` |
| **Download** | [all-grand-slams.ics](all-grand-slams.ics) |

### Individual Tournaments

| Tournament | Surface | Dates | Download |
|-----------|---------|-------|----------|
| Australian Open | Hard | Jan 12 – Feb 1 | [australian-open.ics](australian-open.ics) |
| French Open (Roland-Garros) | Clay | May 24 – Jun 7 | [french-open.ics](french-open.ics) |
| Wimbledon | Grass | Jun 29 – Jul 12 | [wimbledon.ics](wimbledon.ics) |
| US Open | Hard | Aug 31 – Sep 13 | [us-open.ics](us-open.ics) |

---

## How to Subscribe

**Easiest:** Use the [website](https://vinnyab28.github.io/open-sports-cal/) — one click to subscribe or download any calendar.

### Google Calendar
1. Copy the Raw URL above
2. Open [Google Calendar](https://calendar.google.com) → **+** next to "Other calendars" → **From URL**
3. Paste the URL → **Add calendar**

> **Android:** The Google Calendar app cannot subscribe to external URLs directly. Use the desktop steps above — it will sync to your phone automatically.

### Apple Calendar (macOS)
File → **New Calendar Subscription** → paste URL → **Subscribe**

### Apple Calendar (iOS)
Settings → Calendar → Accounts → **Add Account** → Other → **Add Subscribed Calendar** → paste URL

### Microsoft Outlook
Calendar → **Add calendar** → **Subscribe from web** → paste URL → **Import**

---

## Venues

| Tournament | Venue | Location |
|-----------|-------|----------|
| Australian Open | Melbourne Park | Melbourne, Australia |
| French Open | Roland-Garros | Paris, France |
| Wimbledon | All England Lawn Tennis & Croquet Club | London, UK |
| US Open | USTA Billie Jean King National Tennis Center | New York, USA |

---

## Notes

- Grand Slam events span the full tournament window as a single all-day calendar block.
- Individual match schedules are not included — this calendar marks the tournament period only.
- Data source: [ATP Tour](https://www.atptour.com) / [WTA](https://www.wtatennis.com)
